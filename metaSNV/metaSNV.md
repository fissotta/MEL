...
# Example usage: bash split_bams_by_reference_parallel.sh (where split_bams_by_reference_parallel.sh contains the code below)

# Set the number of parallel jobs to run (adjust based on your system's cores)
max_jobs=8

for bam in *.sorted.bam; do
    # Extract the base name without the .sorted.bam extension
    base_name="${bam%.sorted.bam}"
    
    # List references in the BAM file
    references=$(samtools idxstats "$bam" | cut -f1 | grep -v '*')
    
    # Loop through each reference and extract reads to a new BAM file
    for ref in $references; do
        output="${base_name}_${ref}.bam"
        
        # Run samtools in the background
        samtools view -b "$bam" "$ref" > "$output" &
        echo "Started creating $output"
...
wait
echo "All BAM files have been split."
```
